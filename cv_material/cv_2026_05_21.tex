...
\hypersetup{colorlinks=true, urlcolor=accent, linkcolor=accent,
  pdftitle={Haoming Wang -- CV (Qualcomm AI Research)}, pdfauthor={Haoming Wang}}
...
\begin{itemize}[leftmargin=1.1em, itemsep=2pt, topsep=3pt]
  \item \textbf{Spatial Reasoning Performance Improvement (Sep 2025 -- Present).} Improved VLM spatial reasoning via representation-level training and inference-time pipelines: Dirichlet-energy regularizer shaping a VLM latent subspace that encodes 3D scene topology \citeref{pub:vlmlatent}; training-free multi-view fusion for small on-device VLMs, deployed on Jetson Orin, AR Glass, and smartphone \citeref{pub:mosaic}; VLM-based autonomous mobile systems for small-object search \citeref{pub:heven} and infrastructure-assisted navigation \citeref{pub:globalnav}.

  \item \textbf{Spatial Reasoning Benchmarking and Analysis (May 2025 -- Jan 2026).} Established benchmarks and diagnostic methodologies for VLM spatial reasoning: customizable framework generating photo-realistic 3D test scenes with user-controllable complexity axes \citeref{pub:infinibench}; cognitively grounded multi-view benchmark with reasoning-path / latent-state analysis \citeref{pub:remindview}; survey of tasks, benchmarks, and methods \citeref{pub:survey}.
...
%\impactbullet{\textbf{Engineered} a federated AIoT training method that jointly disentangles and corrects for data heterogeneity and device delays via targeted gradient correction, restoring clean convergence on heterogeneous device fleets.}
...
%\impactbullet{\textbf{Designed} a gradient-inversion technique that recovers usable training signal from arbitrarily stale device updates, enabling FL to converge under the unbounded staleness regime real mobile / IoT fleets operate in.}
\end{publist}

% ===== PREPRINTS / UNDER REVIEW =====
\section{Recent Preprints / UNDER REVIEW (First Author)}
...
\me, Wei Gao.\\
\textit{Under Review}, 2026. \href{https://haomingwang645.github.io/pdfs/arxiv26_vlm_latent_shaping.pdf}{[Paper]}\,\href{https://github.com/pittisl/vlm-latent-shaping}{[Code]}
...
\item\label{pub:remindview} \textbf{Reasoning Path and Latent State Analysis for Multi-view Visual Spatial Reasoning: A Cognitive Science Perspective}\\
Qiyao Xue, \meco, Weichen Liu, Shiqi Wang, Yuyang Wu, Wei Gao.\\
\textit{Under Review}, 2026. \href{https://arxiv.org/abs/2512.02340}{[arXiv]}
%\impactbullet{\textbf{Co-authored} ReMindView-Bench, a cognitively grounded multi-view spatial-reasoning benchmark (50K+ VQAs across 15 VLMs) with a joint reasoning-path / latent-state analysis pipeline.}

\item\label{pub:mosaic} \textbf{MosaicThinker: On-Device Visual Spatial Reasoning for Embodied AI via Iterative Construction of Space Representation}\\
...
\item\label{pub:finexl} \textbf{Deciphering Personalization: Towards Fine-Grained Explainability in Natural Language for Personalized Image Generation Models (FineXL)}\\
\me, Wei Gao.\\
\textit{Under Review}, 2025. \href{https://haomingwang645.github.io/pdfs/finexl_paper.pdf}{[Paper]}
%\impactbullet{\textbf{Architected FineXL}, a training-free method that decomposes a personalized image generator's divergence from its base into orthogonal natural-language concepts; cut single-aspect explanation error by 56\% across diffusion, GAN, and autoregressive generators.}
\end{publistcont}
...
\item\label{pub:freeze} \textbf{FreezeAsGuard: Mitigating Illegal Adaptation of Diffusion Models via Selective Tensor Freezing}\\
...
\item\label{pub:heven} \textbf{HEVEN: Small Object Search and Navigation on VLM-Based Autonomous Mobile Systems}\\
...
\item\label{pub:globalnav} \textbf{GlobalNav: Daily Object Navigation in VLM-Based Autonomous Mobile Systems with Aligned Local and Global Views}\\
Anonymous Authors (incl.\ \meanon, 3rd author).\\
\textit{Double-Blind Under Review}, 2026.
%\impactbullet{\textbf{Co-developed} an infrastructure-assisted VLM navigation system aligning ceiling-camera global views with the robot's first-person view; lifted daily-object navigation success from <30\% to 90.91\% in simulation.}



\item\label{pub:survey} \textbf{Spatial Reasoning in Multimodal Large Language Models: A Survey of Tasks, Benchmarks and Methods}\\
...
\textbf{On-Device / Edge:} Jetson Orin (PyTorch + TensorRT), Meta AR Glass, Android (ONNX / NNAPI)
\textbf{Programming:} Python, PyTorch, C/C++.
