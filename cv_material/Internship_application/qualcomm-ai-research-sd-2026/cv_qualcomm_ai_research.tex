...
On-Device Multimodal AI, Vision-Language Models, Visual Spatial Reasoning, Generative-Model Interpretability, Federated Learning, VLM-Based Autonomous Mobile Systems.
...
\begin{itemize}[leftmargin=1.1em, itemsep=2pt, topsep=3pt]
  \item \textbf{(May 2025 -- Present)} Led research on on-device VLM spatial reasoning: photo-realistic 3D benchmark synthesis (CVPR'26 Oral); on-device cross-frame reasoning deployed on Jetson Orin / Meta AR Glass / smartphone with full memory and latency profiling; Dirichlet-energy latent shaping of VLM 3D-topology subspaces.
  \item \textbf{(Nov 2024 -- May 2025)} Drove research on on-device LLM personalization via explainable model selection (MobiSys'25 Xpert), and training-free fine-grained interpretability for diffusion / GAN / autoregressive image generators (FineXL).
  \item \textbf{(Sept 2022 -- Oct 2024)} Designed federated learning systems for heterogeneous AIoT fleets: gradient correction under intertwined data / device heterogeneity (MobiCom'25); federated learning with unbounded device staleness via gradient inversion (AAAI'25).
\end{itemize}
\end{datedentry}
...
\textbf{On-Device / Edge:} Jetson Orin (PyTorch + TensorRT), Meta AR Glass, Android (ONNX / NNAPI), Unitree Go2, RealSense D435i; memory / latency profiling across 3B--32B VLMs.\\[2pt]
